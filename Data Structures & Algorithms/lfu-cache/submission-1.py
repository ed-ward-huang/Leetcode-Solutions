...
'''
Time Comlexity: Since we used two dicts and doubly linked list for lru part, put and get is O(1)
Space complexity: O(cpacity), self.store() only stores at max -> capacity and cleans up, 
same for freq, cleans up, so at max stores -> capacity
'''
